...
def get_emb(wavs, model, layer=-1):
	es = []
	for waveform in wavs:
		e = embed(waveform, model, layer)
		es.append(e)
	return es


def make_avg_gallery(embs, win):
	ma_embs = []
	for e in embs:
		for i in range(len(e)-win+1):
			e[i] = e[i:i+win].mean(0)
		e = e[:-win+1]
		ma_embs.append(e)
	return np.vstack(ma_embs)


def make_cat_gallery(embs, win):
	ma_embs = []
	for e in embs:
		e_new = np.zeros((e.shape[0] - win + 1, e.shape[1] * win))
		for i in range(len(e_new)):
			ew = e[i:i+win]
			e_new[i] = ew.flatten()
		ma_embs.append(e_new)
	return np.vstack(ma_embs)


def make_gallery(embs, win, avg):
	if avg:
		return make_avg_gallery(embs, win)
	else:
		return make_cat_gallery(embs, win)


def embed(y, model, extract_layer=-1):
	with torch.no_grad():
		model.eval()
